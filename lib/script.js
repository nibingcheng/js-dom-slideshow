...
*/
let jsGalleryElement = document.querySelector(".js-gallery");
let jsGalleryItems = document.querySelectorAll('.js-gallery-item');
...
let slideWidth = jsGalleryItems[0].getBoundingClientRect().width;
...
transitionSlide = () => {
  // console.log('Called!');
    if (currentSlide < slideCount) {
      jsGalleryElement.style.transform = "translateX(" + (currentSlide*slideWidth*(-1)).toString() +  "px)";
      currentSlide++;
    }
    else {
      jsGalleryElement.style.transform = "translateX(" + "0"  + ")";
      currentSlide = 1;
    }
}

setInterval(transitionSlide, 800);
